...
const CASES = {
  ...
  'men-jackets': {
    ...
    adjective: 'индивидуальная',
    intro: 'Мужская куртка строится под нужный объём, длину и функциональность. Мы продумываем всё до мельчайших деталей.',
...
function renderHtml(page, copy) {
  ...
  html = html
    ...
    .replace(/Опишите женский жакет или прикрепите фото референса\.\s+Я задам уточняющие вопросы и\s+посчитаю ориентир по пошиву\./g, `Опишите ${copy.accusative} или прикрепите фото референса. Я задам уточняющие вопросы и посчитаю ориентир по пошиву.`)
